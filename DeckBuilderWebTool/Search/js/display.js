class DisplayManager {
    ...
    isCardLegalInActiveDeck(card) {
        const activeDeck = window.deckBuilder?.getActiveDeck();
        console.log('Active deck:', activeDeck);
        
        if (!activeDeck || !activeDeck.format) return true;
        return card.legalities[activeDeck.format.toLowerCase()] === 'legal';
    }
    ...
    createCardElement(card) { 
        ...
        // Add illegal overlay if card is not legal
        if (!this.isCardLegalInActiveDeck(card)) {
            const illegalOverlay = document.createElement('div');
            illegalOverlay.className = 'illegal-overlay';
            illegalOverlay.innerHTML = '<span>ILLEGAL</span>';
            div.appendChild(illegalOverlay);
        }
...
class PreviewManager {
    ...
    showPreview(card) {
        ...
        const cardDetails = this.previewElement.querySelector('.card-details');
        const addButton = document.createElement('button');
        addButton.className = 'add-to-deck-button';
        addButton.innerHTML = '+';
        
        if (this.isCardLegalInActiveDeck(card)) {
            cardDetails.appendChild(addButton);
   ...
   Object.entries(card.legalities).forEach(([format, status]) => {
    ...
    async updatePriceInfo(card) {
        console.log('Updating price info for card:', card);
   ...
   findCheapestPrice(variants) {
    ...
    async formatPrices(card) {
        ...
        if (card.flavor_name) {
            // For flavor cards, show cheapest of both versions
            const flavorVersions = variants.filter(v => v.flavor_name);
            const cheapestFlavor = this.findCheapestPrice(flavorVersions);
            prices.push(`Cheapest Flavor Version: $${cheapestFlavor === Infinity ? 'N/A' : cheapestFlavor.toFixed(2)}`);
            ...
            // For regular cards, show only the cheapest price
            const cheapestPrice = this.findCheapestPrice(variants);
            prices.push(`Cheapest Available: $${cheapestPrice === Infinity ? 'N/A' : cheapestPrice.toFixed(2)}`);
        }
