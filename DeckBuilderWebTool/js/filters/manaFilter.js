...
class ManaFilter {
    ...
    initializeManaFilters() {
        ...
        const manaCostToggle = this.createToggleSwitch('Exactly These Colors', 'Including Colors');
        ...
        const colorIdentityToggle = this.createToggleSwitch('Exactly This Identity', 'Including Identity');
        ...
        colorIdentityFilter.parentNode.appendChild(this.createExactCostSection());
    }
    ...
    setupSearchButton() {
        ...
        searchButton.addEventListener('click', async () => {
            ...
            // Otherwise use the color-based search
                ...
                const isIncludeMode = document.getElementById('manaCostToggle').checked;
                if (manaCostSymbols.includes('C')) {
                    searchParts.push('-m:/[WUBRG]/ -o:devoid');
                } else {
                    if (!isIncludeMode) {
                        // Use regex to match both regular and Phyrexian versions
                        const symbolsWithPhyrexian = manaCostSymbols.map(symbol => 
                            `m:/[{]${symbol}([/]P)?[}]/`
                        ).join(' ');
                        searchParts.push(`${symbolsWithPhyrexian} -o:devoid`);
                    } else {
                        searchParts.push(`m:${manaCostSymbols.join('')} -o:devoid`);
                    }
                }
            }
            ...
            if (colorIdentitySymbols.length > 0) {
                const isIncludeMode = document.getElementById('colorIdentityToggle').checked;
                searchParts.push(!isIncludeMode ? `id=${colorIdentitySymbols.join('')}` : `id:${colorIdentitySymbols.join('')}`);
            }
    ...
    createToggleSwitch(label = 'Exactly', checkedLabel = 'Including') {
